...
Elm.KeyPresses = Elm.KeyPresses || {};
Elm.KeyPresses.make = function (_elm) {
   "use strict";
   _elm.KeyPresses = _elm.KeyPresses || {};
   if (_elm.KeyPresses.values) return _elm.KeyPresses.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Char = Elm.Char.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var display = function (keyCode) {    return $Graphics$Element.show($Char.fromCode(keyCode));};
   var containerShow = function (signal) {    return A4($Graphics$Element.container,1100,200,$Graphics$Element.middle,signal);};
   var main = A2($Signal.map,function (_p0) {    return containerShow(display(_p0));},$Keyboard.presses);
   return _elm.KeyPresses.values = {_op: _op,containerShow: containerShow,main: main,display: display};
};
