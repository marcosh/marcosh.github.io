...
Elm.HtmlToUnicode = Elm.HtmlToUnicode || {};
Elm.HtmlToUnicode.make = function (_elm) {
   "use strict";
   _elm.HtmlToUnicode = _elm.HtmlToUnicode || {};
   if (_elm.HtmlToUnicode.values) return _elm.HtmlToUnicode.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var convertFriendlyCode = F3(function (pre,post,list) {    return $List.concat(_U.list([pre,list,post]));});
   var convertNumericalCode = F3(function (pre,post,list) {
      var string = $String.fromList(list);
      var keyCodeResult = $String.toInt(string);
      var _p0 = keyCodeResult;
      if (_p0.ctor === "Err") {
            return $List.concat(_U.list([pre,list,post]));
         } else {
            return _U.list([$Char.fromCode(_p0._0)]);
         }
   });
   var noAmpUnicodeConverter = F3(function (pre,post,list) {
      var _p1 = list;
      if (_p1.ctor === "[]") {
            return _U.list([pre,post]);
         } else {
            var _p3 = _p1._1;
            var _p2 = _p1._0;
            return _U.eq(_p2,_U.chr("#")) ? A3(convertNumericalCode,_U.list([pre,_p2]),_U.list([post]),_p3) : A3(convertFriendlyCode,
            _U.list([pre,_p2]),
            _U.list([post]),
            _p3);
         }
   });
   var unicodeConverter = F2(function (post,list) {
      var _p4 = list;
      if (_p4.ctor === "[]") {
            return _U.list([post]);
         } else {
            return A3(noAmpUnicodeConverter,_p4._0,post,_p4._1);
         }
   });
   var parser = F3(function (charsToBeParsed,charsOnParsing,charsParsed) {
      parser: while (true) {
         var tailToBeParsed = A2($Maybe.withDefault,_U.list([]),$List.tail(charsToBeParsed));
         var maybeNextChar = $List.head(charsToBeParsed);
         var _p5 = maybeNextChar;
         if (_p5.ctor === "Nothing") {
               return charsParsed;
            } else {
               var _p6 = _p5._0;
               if (_U.eq(_p6,_U.chr("&"))) {
                     var _v4 = tailToBeParsed,_v5 = _U.list([_p6]),_v6 = charsParsed;
                     charsToBeParsed = _v4;
                     charsOnParsing = _v5;
                     charsParsed = _v6;
                     continue parser;
                  } else if (_U.eq(_p6,_U.chr(";"))) {
                        var _v7 = tailToBeParsed,_v8 = _U.list([]),_v9 = A2($List.append,charsParsed,A2(unicodeConverter,_p6,charsOnParsing));
                        charsToBeParsed = _v7;
                        charsOnParsing = _v8;
                        charsParsed = _v9;
                        continue parser;
                     } else if ($Basics.not($List.isEmpty(charsOnParsing))) {
                           var _v10 = tailToBeParsed,_v11 = A2($List.append,charsOnParsing,_U.list([_p6])),_v12 = charsParsed;
                           charsToBeParsed = _v10;
                           charsOnParsing = _v11;
                           charsParsed = _v12;
                           continue parser;
                        } else {
                           var _v13 = tailToBeParsed,_v14 = _U.list([]),_v15 = A2($List.append,charsParsed,_U.list([_p6]));
                           charsToBeParsed = _v13;
                           charsOnParsing = _v14;
                           charsParsed = _v15;
                           continue parser;
                        }
   ...
   var convertChars = function (list) {    return A3(parser,list,_U.list([]),_U.list([]));};
   var convert = function (string) {    return $String.concat(A2($List.map,$String.fromChar,convertChars($String.toList(string))));};
   return _elm.HtmlToUnicode.values = {_op: _op
                                      ,convert: convert
                                      ,convertChars: convertChars
                                      ,parser: parser
                                      ,unicodeConverter: unicodeConverter
                                      ,noAmpUnicodeConverter: noAmpUnicodeConverter
                                      ,convertNumericalCode: convertNumericalCode
                                      ,convertFriendlyCode: convertFriendlyCode};
};
Elm.Talk = Elm.Talk || {};
Elm.Talk.make = function (_elm) {
   "use strict";
   _elm.Talk = _elm.Talk || {};
   if (_elm.Talk.values) return _elm.Talk.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $HtmlToUnicode = Elm.HtmlToUnicode.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var stubTalk = function (talk) {    return {ctor: "_Tuple2",_0: talk.stub,_1: talk};};
   var speakerDecoder = A2($Json$Decode.object1,$Basics.identity,A2($Json$Decode._op[":="],"speaker_name",$Json$Decode.string));
   var spanWrap = function (string) {    return A2($Html.span,_U.list([]),_U.list([$Html.text(string)]));};
   var divWrap = function (string) {    return A2($Html.div,_U.list([]),_U.list([$Html.text(string)]));};
   var update = F2(function (action,talk) {
      var _p0 = action;
      if (_p0.ctor === "ClickTitle") {
            return talk;
         } else {
            return _U.update(talk,{description: "We got a new description!"});
         }
   });
   var ClickDescription = {ctor: "ClickDescription"};
   var ClickTitle = {ctor: "ClickTitle"};
   var view = F2(function (address,talk) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("talk")]),
      _U.list([A2($Html.div,
              _U.list([A2($Html$Events.onClick,address,ClickTitle)]),
              _U.list([$Html.text($HtmlToUnicode.convert(talk.title))
                      ,$Html.text(" by ")
                      ,A2($Html.span,_U.list([]),A2($List.map,spanWrap,A2($List.intersperse,", ",talk.speakers)))]))
              ,A2($Html.div,_U.list([A2($Html$Events.onClick,address,ClickDescription)]),_U.list([$Html.text($HtmlToUnicode.convert(talk.description))]))]));
   });
   var Talk = F6(function (a,b,c,d,e,f) {    return {title: a,description: b,stub: c,slides_link: d,uri: e,speakers: f};});
   var talkDecoder = A7($Json$Decode.object6,
   Talk,
   A2($Json$Decode._op[":="],"talk_title",$Json$Decode.string),
   A2($Json$Decode._op[":="],"talk_description",$Json$Decode.string),
   A2($Json$Decode._op[":="],"stub",$Json$Decode.string),
   A2($Json$Decode._op[":="],"slides_link",$Json$Decode.string),
   A2($Json$Decode._op[":="],"website_uri",$Json$Decode.string),
   A2($Json$Decode._op[":="],"speakers",$Json$Decode.list(speakerDecoder)));
   return _elm.Talk.values = {_op: _op
                             ,Talk: Talk
...
Elm.Config.make = function (_elm) {
   ...
   var _op = {};
   var apiPort = 6081;
   var apiHost = "95.110.184.16";
   var apiProtocol = "http";
   return _elm.Config.values = {_op: _op,apiProtocol: apiProtocol,apiHost: apiHost,apiPort: apiPort};
};
Elm.Erl = Elm.Erl || {};
Elm.Erl.make = function (_elm) {
   ...
   var $new = {protocol: "",username: "",password: "",host: _U.list([]),path: _U.list([]),hasTrailingSlash: false,port$: 0,hash: "",query: $Dict.empty};
   var hashToString = function (url) {    return $String.isEmpty(url.hash) ? "" : A2($Basics._op["++"],"#",url.hash);};
   var trailingSlashComponent = function (url) {    return _U.eq(url.hasTrailingSlash,true) ? "/" : "";};
   ...
   var toString = function (url) {
      ...
      var trailingSlash$ = trailingSlashComponent(url);
      ...
      A2($Basics._op["++"],
      host$,
      A2($Basics._op["++"],port$,A2($Basics._op["++"],path$,A2($Basics._op["++"],trailingSlash$,A2($Basics._op["++"],query$,hash))))));
   };
   ...
   var hasTrailingSlashFromAll = function (str) {    return A2($Regex.contains,$Regex.regex("/$"),extractPath(str));};
   ...
   var Url = F9(function (a,b,c,d,e,f,g,h,i) {    return {protocol: a,username: b,password: c,host: d,port$: e,path: f,hasTrailingSlash: g,hash: h,query: i};});
...
Elm.Conference = Elm.Conference || {};
Elm.Conference.make = function (_elm) {
   "use strict";
   _elm.Conference = _elm.Conference || {};
   if (_elm.Conference.values) return _elm.Conference.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm),
   $Talk = Elm.Talk.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var idConference = function (conference) {    return {ctor: "_Tuple2",_0: conference.urlFrienlyName,_1: conference};};
   var decoder = A2($Json$Decode.object1,
   $Basics.identity,
   A2($Json$Decode._op[":="],"talks",$Json$Decode.list(A2($Json$Decode.map,$Talk.stubTalk,$Talk.talkDecoder))));
   var filterTalksWithSlides = function (maybeTalks) {
      var _p0 = maybeTalks;
      if (_p0.ctor === "Nothing") {
            return _U.list([]);
         } else {
            return A2($List.filter,function (_p1) {    var _p2 = _p1;return $Basics.not($String.isEmpty(_p2._1.slides_link));},_p0._0);
         }
   };
   var Forward = F2(function (a,b) {    return {ctor: "Forward",_0: a,_1: b};});
   var viewTalk = F2(function (address,_p3) {    var _p4 = _p3;return A2($Talk.view,A2($Signal.forwardTo,address,Forward(_p4._0)),_p4._1);});
   var view = F2(function (address,conference) {
      if ($List.isEmpty(conference.talks)) return $Html.text(""); else {
            var conferenceTalks = A2($List.map,viewTalk(address),conference.talks);
            var conferenceName = A2($Html.h2,_U.list([]),_U.list([$Html.text(conference.name)]));
            var iconPath = function () {
               var _p5 = conference.icon;
               if (_p5.ctor === "Just") {
                     return A2($Basics._op["++"],"https://joind.in/inc/img/event_icons/",_p5._0);
                  } else {
                     return "https://joind.in/img/event_icons/none.png";
                  }
            }();
            var conferenceLogo = A2($Html.img,_U.list([$Html$Attributes.src(iconPath),$Html$Attributes.$class("logo")]),_U.list([]));
            var conferenceHeader = A2($Html.div,_U.list([$Html$Attributes.$class("conferenceHeader")]),_U.list([conferenceLogo,conferenceName]));
            return A2($Html.div,_U.list([$Html$Attributes.$class("conference")]),A2($List._op["::"],conferenceHeader,conferenceTalks));
         }
   });
   var TalksRetrieved = function (a) {    return {ctor: "TalksRetrieved",_0: a};};
   var retrieveTalks = function (uri) {    return $Effects.task(A2($Task.map,TalksRetrieved,$Task.toMaybe(A2($Http.get,decoder,uri))));};
   var realTalksUri = function (talksUri) {
      var splittedApiHost = A2($String.split,".",$Config.apiHost);
      return function (uri) {
         return A2($Basics._op["++"],uri,"?verbose=yes");
      }($Erl.toString(function (uri) {
         return _U.update(uri,{host: splittedApiHost,protocol: $Config.apiProtocol,port$: $Config.apiPort});
      }($Erl.parse(talksUri))));
   };
   var Conference = F8(function (a,b,c,d,e,f,g,h) {    return {name: a,urlFrienlyName: b,description: c,uri: d,joindinUri: e,icon: f,talksUri: g,talks: h};});
   var build = F7(function (name,urlFrienlyName,description,uri,joindinUri,icon,talksUri) {
      return A8(Conference,name,urlFrienlyName,description,uri,joindinUri,icon,realTalksUri(talksUri),_U.list([]));
   });
   var init = F7(function (name,urlFrienlyName,description,uri,joindinUri,icon,talksUri) {
      return {ctor: "_Tuple2",_0: A7(build,name,urlFrienlyName,description,uri,joindinUri,icon,talksUri),_1: retrieveTalks(realTalksUri(talksUri))};
   });
   var decoderConference = A8($Json$Decode.object7,
   build,
   A2($Json$Decode._op[":="],"name",$Json$Decode.string),
   A2($Json$Decode._op[":="],"url_friendly_name",$Json$Decode.string),
   A2($Json$Decode._op[":="],"description",$Json$Decode.string),
   A2($Json$Decode._op[":="],"website_uri",$Json$Decode.string),
   A2($Json$Decode._op[":="],"uri",$Json$Decode.string),
   $Json$Decode.maybe(A2($Json$Decode._op[":="],"icon",$Json$Decode.string)),
   A2($Json$Decode._op[":="],"talks_uri",$Json$Decode.string));
   var addTalks = F2(function (conference,maybeTalks) {
      return A8(Conference,
      conference.name,
   ...
   var update = F2(function (action,conference) {
      var _p6 = action;
      if (_p6.ctor === "TalksRetrieved") {
            return {ctor: "_Tuple2",_0: A2(addTalks,conference,_p6._0),_1: $Effects.none};
         } else {
            return {ctor: "_Tuple2",_0: conference,_1: $Effects.none};
         }
   });
   return _elm.Conference.values = {_op: _op
                                   ,Conference: Conference
                                   ,realTalksUri: realTalksUri
                                   ,init: init
                                   ,build: build
                                   ,TalksRetrieved: TalksRetrieved
                                   ,Forward: Forward
                                   ,update: update
                                   ,addTalks: addTalks
                                   ,filterTalksWithSlides: filterTalksWithSlides
                                   ,view: view
                                   ,viewTalk: viewTalk
                                   ,retrieveTalks: retrieveTalks
                                   ,decoder: decoder
                                   ,decoderConference: decoderConference
                                   ,idConference: idConference};
};
Elm.Joindin = Elm.Joindin || {};
Elm.Joindin.make = function (_elm) {
   "use strict";
   _elm.Joindin = _elm.Joindin || {};
   if (_elm.Joindin.values) return _elm.Joindin.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Conference = Elm.Conference.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   ...
   var decoder = A2($Json$Decode.object1,
   $Basics.identity,
   A2($Json$Decode._op[":="],"events",$Json$Decode.list(A2($Json$Decode.map,$Conference.idConference,$Conference.decoderConference))));
   var Forward = F2(function (a,b) {    return {ctor: "Forward",_0: a,_1: b};});
   var retrieveTalkEffect = function (_p0) {    var _p1 = _p0;return A2($Effects.map,Forward(_p1._0),$Conference.retrieveTalks(_p1._1.talksUri));};
   var viewConference = F2(function (address,_p2) {    var _p3 = _p2;return A2($Conference.view,A2($Signal.forwardTo,address,Forward(_p3._0)),_p3._1);});
   var view = F2(function (address,joindin) {
      var foot = A2($Html.footer,
      _U.list([]),
      _U.list([A2($Html.div,
              _U.list([]),
              _U.list([$Html.text("This page is realized using the data exposed by the ")
                      ,A2($Html.a,_U.list([$Html$Attributes.href("https://joind.in/")]),_U.list([$Html.text("Joind.in")]))
                      ,$Html.text(" API\'s.")]))
              ,A2($Html.div,
              _U.list([]),
              _U.list([$Html.text("This page is made using ")
                      ,A2($Html.a,_U.list([$Html$Attributes.href("http://elm-lang.org/")]),_U.list([$Html.text("ELM")]))
                      ,$Html.text(", a rective funtional programming language that runs in the browser. You can find the source code on ")
                      ,A2($Html.a,
                      _U.list([$Html$Attributes.href("https://github.com/marcosh/elm-joindin")]),
                      _U.list([$Html.text("https://github.com/marcosh/elm-joindin")]))
                      ,$Html.text(", contributions are more than welcome!")]))]));
      var conferences = A2($List.map,viewConference(address),joindin.conferences);
      var title = A2($Html.header,
      _U.list([]),
      _U.list([A2($Html.h1,
              _U.list([]),
              _U.list([$Html.text("The "),A2($Html.span,_U.list([$Html$Attributes.$class("cursive")]),_U.list([$Html.text("slides")])),$Html.text(" archive")]))
              ,A2($Html.h3,
              _U.list([$Html$Attributes.$class("subtitle")]),
              _U.list([$Html.text("Did you miss a conference? Here you will find all the latest slides uploaded on ")
                      ,A2($Html.a,_U.list([$Html$Attributes.href("https://joind.in/")]),_U.list([$Html.text("Joind.in")]))
                      ,$Html.text(" by the most amazing speakers in the wild!")]))]));
      return A2($Html.div,_U.list([]),$List.concat(_U.list([A2($List._op["::"],title,conferences),_U.list([foot])])));
   });
   var RetrieveTalks = {ctor: "RetrieveTalks"};
   var ConferencesRetrieved = function (a) {    return {ctor: "ConferencesRetrieved",_0: a};};
   var retrieveConferences = function (uri) {    return $Effects.task(A2($Task.map,ConferencesRetrieved,$Task.toMaybe(A2($Http.get,decoder,uri))));};
   var Joindin = function (a) {    return {conferences: a};};
   var init = function (conferencesUri) {    return {ctor: "_Tuple2",_0: Joindin(_U.list([])),_1: retrieveConferences(conferencesUri)};};
   var addConferences = F2(function (joindin,maybeConferences) {    return Joindin(A2($Maybe.withDefault,_U.list([]),maybeConferences));});
   var update = F2(function (action,joindin) {
      var _p4 = action;
      switch (_p4.ctor)
      {case "ConferencesRetrieved": return {ctor: "_Tuple2",_0: A2(addConferences,joindin,_p4._0),_1: $Effects.task($Task.succeed(RetrieveTalks))};
         case "RetrieveTalks": return {ctor: "_Tuple2",_0: joindin,_1: $Effects.batch(A2($List.map,retrieveTalkEffect,joindin.conferences))};
         default: var _p10 = _p4._0;
           var updateConference = function (_p5) {
              var _p6 = _p5;
              var _p8 = _p6._0;
              if (_U.eq(_p10,_p8)) {
                    var _p7 = A2($Conference.update,_p4._1,_p6._1);
                    var newConference = _p7._0;
                    var effect = _p7._1;
                    return {ctor: "_Tuple2",_0: {ctor: "_Tuple2",_0: _p8,_1: newConference},_1: A2($Effects.map,Forward(_p10),effect)};
                 } else return {ctor: "_Tuple2",_0: _p6,_1: $Effects.none};
           };
           var _p9 = $List.unzip(A2($List.map,updateConference,joindin.conferences));
           var newConferences = _p9._0;
           var effectsList = _p9._1;
           return {ctor: "_Tuple2",_0: _U.update(joindin,{conferences: newConferences}),_1: $Effects.batch(effectsList)};}
   });
   return _elm.Joindin.values = {_op: _op
                                ,Joindin: Joindin
                                ,init: init
                                ,ConferencesRetrieved: ConferencesRetrieved
                                ,RetrieveTalks: RetrieveTalks
                                ,Forward: Forward
                                ,update: update
                                ,addConferences: addConferences
                                ,retrieveTalkEffect: retrieveTalkEffect
                                ,view: view
                                ,viewConference: viewConference
                                ,retrieveConferences: retrieveConferences
                                ,decoder: decoder};
};
Elm.JoindinMain = Elm.JoindinMain || {};
Elm.JoindinMain.make = function (_elm) {
   "use strict";
   _elm.JoindinMain = _elm.JoindinMain || {};
   if (_elm.JoindinMain.values) return _elm.JoindinMain.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Config = Elm.Config.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $Joindin = Elm.Joindin.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var _op = {};
   var app = function () {
      var apiUri = A2($Basics._op["++"],
      $Config.apiProtocol,
      A2($Basics._op["++"],
      "://",
      A2($Basics._op["++"],
      $Config.apiHost,
      A2($Basics._op["++"],":",A2($Basics._op["++"],$Basics.toString($Config.apiPort),"/v2.1/events?filter=past&verbose=yes")))));
      return $StartApp.start({init: $Joindin.init(apiUri),update: $Joindin.update,view: $Joindin.view,inputs: _U.list([])});
   }();
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   return _elm.JoindinMain.values = {_op: _op,app: app,main: main};
};
